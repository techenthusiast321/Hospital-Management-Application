...
const bookAppointment = async (req, res) => {
  try {
    ...
    console.log("first", slots_booked);
    ...
    if (slots_booked[slotDate]) {
      ...
      slots_booked[slotDate] = [slotTime]; // <-- Fix here
      console.log("firiri", slots_booked);
    }

    const userData = await userModel.findById(userId).select("-password");
    // console.log("sec", userData)
    if (!userData)
